...
(deftest test-basic-adjusting-target
  (let [blocks [{:header {:timestamp 0 :target 100}}
                {:header {:timestamp 60 :target 100}}]]
    (is (= 100 (adjusted-target blocks 60))))
  (let [blocks [{:header {:timestamp 0 :target 100}}
                {:header {:timestamp 55 :target 100}}
                {:header {:timestamp 108 :target 100}}]]
    (is (= 89 (adjusted-target blocks 60)))))

(deftest test-max-target-adjustment
  (let [blocks [{:header {:timestamp 0 :target 100}}
                {:header {:timestamp 120 :target 100}}
                {:header {:timestamp 240 :target 100}}]]
    ;; Blocks are spacing 120 s against desired freq of 60
    ;; so increases by the max adjustment of 15%
    (is (= 114 (adjusted-target blocks 60)))))

(deftest test-min-target-adjustment
  (let [blocks [{:header {:timestamp 0 :target 100}}
                {:header {:timestamp 30 :target 100}}
                {:header {:timestamp 60 :target 100}}]]
    ;; blocks are spacing 30 s against desired freq of 60,
    ;; so we should reduce the target by max adjustment of 15%
    (is (= 85 (adjusted-target blocks 60)))))
...
;;   (is (= 126992132780946360000000000000000000000000000000000000000000000000000000N
